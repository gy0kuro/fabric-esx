#
##
# Script de configuration ESXi PEDAGO / tgr 04122014.1 (v1) / ddm 22.06.2015.1
##
# Documentation :
#
...
#   - avoir modified le vSwitch0 en vSwitch1 dans /etc/vmware/config
#
#   GW  :   194.254.103.97
#   DNS :   194.199.46.5
#   DNS :   194.199.46.6
...
def esxfreshinstall():
        ...
        abort_on_prompts=False
            ...
            # VARIABLES A TUNER !!!!
            #
            ip_hb = '172.26.178.209'
            ip_vmotion = '192.168.230.57'
            ip_iscsi1 = '172.26.178.210'
            ip_iscsi2 = '172.26.178.211'
            ip_iscsi3 = '172.26.178.212'
            ip_iscsi4 = '172.26.178.213'
            # serveurdns = 172.30.176.100
            ...
            # prompt hostname
            run('esxcli system hostname set --host='+nomesxquilestbien+'')
            run('esxcli network ip dns server add -s 172.30.176.100')
            run('esxcli network ip dns server add -s 172.30.191.245')
            run('esxcli network ip dns search add -d "in.ac-amiens.fr"')
            run('esxcli network ip dns search add -d "ac-amiens.fr"')

            # ntp
            run('rm /etc/ntp.conf && touch /etc/ntp.conf && chmod 644 /etc/ntp.conf && chmod -t /etc/ntp.conf')
            run('''
            cat >/etc/ntp.conf <<"EOF"
            restrict default kod nomodify notrap nopeer
            restrict 127.0.0.1
            server 172.30.176.100
            ...
            #
            # iscsi-nashville-1.in.ac-amiens.fr has address 172.26.178.210
            # iscsi-nashville-2.in.ac-amiens.fr has address 172.26.178.211
            # iscsi-nashville-3.in.ac-amiens.fr has address 172.26.178.212
            # iscsi-nashville-4.in.ac-amiens.fr has address 172.26.178.213
            #
            # iscsi-nashville-hb.in.ac-amiens.fr has address 172.26.178.209
            #
            # nashville.in.ac-amiens.fr has address 172.30.186.129
            #
            # vmotion-nashville.in.ac-amiens.fr has address 192.168.230.57
            #
